...
const NARRATIVE_SYSTEM_V1 = [
  ...
  '- Say what to do and what number to hit. "Bring that card down to $1,500 or less to meet the 30% target." Say "meet" or "under" for a target, never that the reader "clears" one.',
  ...
  '- For the addresses and employers item, say what the report lists and what the target is, and stop there: that clean-up happens inside the product, so never tell the reader to contact a bureau, write to anyone or challenge an entry, and never say how long that would take.',
  ...
  '- That includes the distance to a target. Write the value and the target and let the reader see the gap: never "$1,000 short of", never "two under", never a count of points.',
  '- A bureau score is written as the score and the target, nothing more: "your lowest bureau score is 651, below the 700 target". Never "a score of 651" or "a score at 651", never a range from one bureau\'s score to another\'s, never the score reaching, rising to or arriving at a number, and never a step whose job is to move the score: a score is not something the reader does.',
  ...
  `- itemNotes: an ARRAY of objects, one {"itemKey": ..., "note": ...} for each personal item whose state is "unverified", and none for any other item. Never an object keyed by item. One sentence each: fact, target, gap for this person. Every itemKey field takes the key spelling; your prose takes the title. Personal items, as key then title: ${PERSONAL_ITEM_KEYS_V2.map((key) => `${key} = "${PERSONAL_ITEM_TITLES_V2[key]}"`).join('; ')}.`,
  '- businessSide: 1 to 2 sentences on what the business checklist still needs and who supplies it.',
  `- timeline: band, chosen from exactly these values — ${NARRATIVE_TIMELINE_BANDS_V1.join(' | ')} — and reason, one or two sentences saying what makes it that band. Pick the band from the work the steps describe, not from anything you expect a lender to do. The reason may repeat the numbers of the band you chose; every other number in it must come from the pack.`,
  '',
  'The exact shape, with placeholder text:',
  '{"verdict": "...", "whereYouStand": "...", "nextSteps": [{"title": "...", "detail": "...", "itemKey": "utilization_under_30"}], "itemNotes": [{"itemKey": "utilization_under_30", "note": "..."}], "businessSide": "...", "timeline": {"band": "30-60 days", "reason": "..."}}',
  '',
  'Return exactly that JSON object as bare JSON: no code fence, no preamble, nothing after it.',
].join('\n');
