-- R1C-03: reclaim expired background-job leases under the existing three-attempt cap.

create index if not exists background_jobs_expired_lease_idx
  on public.background_jobs(lease_until, created_at, id)
...
begin
  ...
  for v_before in
    ...
    where (
        ...
        and candidate.lease_until <= pg_catalog.now()
      )
...
revoke all on function public.claim_background_jobs(text, integer, integer)
  from public, anon, authenticated;
revoke all on function public.claim_background_job(uuid, text, integer)
  from public, anon, authenticated;
