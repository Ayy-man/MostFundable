...
select throws_ok(
  ...
  'P0001',
  'stage_transition_not_allowed',
  'a manual move cannot step back a stage'
);
...
select throws_ok(
  ...
  'P0001',
  'stage_transition_not_allowed',
  'a manual move cannot skip a stage from the middle of the tracker'
);

select is(
  (select stage from public.clients where id = '43600000-0000-4000-8000-000000000201'),
  'optimization'::public.client_stage,
  'refused moves leave the client where it was'
);
...
select is(
  ...
  1,
  'only the one allowed manual move creates history'
);
...
select is(
  ...
  1,
  'only the one allowed manual move creates audit evidence'
);
