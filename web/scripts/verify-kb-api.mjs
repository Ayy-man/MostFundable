...
try {
  ...
  equal(consumer.status, 200, "consumer answer succeeds"); equal(consumer.payload?.status, "answered", "consumer answer is grounded"); equal(consumer.payload?.identity, "AI knowledge assistant", "consumer answer identifies AI"); check(Array.isArray(consumer.payload?.citations) && consumer.payload.citations.length > 0, "consumer answer has citations");
