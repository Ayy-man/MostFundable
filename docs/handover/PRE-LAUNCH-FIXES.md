...
| B7 | Fixed. The operator inbox receives the timeline feature flag. | Upload requests and the conversation timeline appear in the inbox when the flag is on. | — | `web/src/components/surfaces/operator.tsx:9296`; `web/src/components/operator/inbox/inbox-contract.test.ts:104-108` |
...
| --- | --- | --- | --- | --- |
| C1 | The replay-safety check for credit pulls ignores its argument and always returns true, which bypasses the guard that migration 378 added against buying the same report twice. | Double bureau billing on a retried analysis. | Make the function return the driver's real answer, or remove the claim that GET pulls are free and let the durable record decide. | `web/src/lib/crs/adapter.ts:44-47`; `web/src/lib/analysis/worker.ts:245`; `supabase/migrations/378_r5c04_crs_pull_operations.sql:6-33` |
| C2 | Monthly revenue accrual refuses to compute when any consumer subscription is on Stripe. | Revenue share stops the day real payments start. | Implement the paid-invoice evidence path for the Stripe arm. | `web/src/lib/revenue/accruals.ts:51-53` |
...
| C4 | Removed. The unsupported daily analysis schedule job is no longer catalogued or accepted by the background-job queue. | — | — | `web/src/lib/jobs/definitions.ts`; `supabase/migrations/436_remove_analysis_schedule_due_job.sql` |
