...
type Dependencies = {
  ...
  createPortal(orgId: string, requestUrl: string): Promise<{ url: string }>;
...
function failure(error: unknown): Response {
  ...
  // R5B-03, the sibling of the checkout mapper: the unknown arm records and correlates.
  const correlationId = recordRouteFailure({
    cause: error,
    code: "billing_unavailable",
    status: 500,
    surface: "api.billing.portal",
  });
  return json(withCorrelationId({ error: { code: "billing_unavailable" } }, correlationId), 500);
}

export async function handlePortal(request: Request, dependencies: Dependencies): Promise<Response> {
  ...
  try {
    ...
    return json(await dependencies.createPortal(actor.orgId, request.url));
  } catch (error) {
...
export async function POST(request: Request): Promise<Response> {
  ...
  const [{ requireOrgMember }, { assertTenantWriteAllowed }, { createHostedPortal }] = await Promise.all([
    import("@/lib/auth/session"),
    import("@/lib/tenancy/wall"),
    import("@/lib/billing/service-operations"),
  ]);
  return handlePortal(request, { assertTenantWriteAllowed, createPortal: createHostedPortal, requireOrgMember });
}
