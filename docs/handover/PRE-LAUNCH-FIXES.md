...
| B7 | The operator inbox timeline is hard-wired off even when the timeline flag is on. | Upload requests and the conversation timeline do not appear in the inbox. | Pass the flag through instead of `false`. | `web/src/components/surfaces/operator.tsx:9287` |
