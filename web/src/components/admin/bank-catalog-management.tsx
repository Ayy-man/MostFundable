                  ...
                  <Table className="min-w-[980px]">
                    ...
                    <TableBody>{filtered.map((bank) => (
                      <TableRow key={bank.bankRef}>
                        ...
                        <TableCell className="min-w-48 max-w-64 whitespace-normal text-xs leading-5">{bank.products.length ? bank.products.join(", ") : "No products recorded"}</TableCell>
                        ...
                        <TableCell className="text-right"><div className="grid grid-cols-[auto_7.5rem] justify-end gap-1"><Button aria-label={`Edit ${bank.name}`} disabled={busy !== null} onClick={() => { setNotice(null); setEditor(editorFor(bank)); }} size="sm" variant="outline">Edit</Button>{bank.isActive ? <Button aria-label={`Archive ${bank.name}`} disabled={busy !== null} onClick={() => setArchiveCandidate(bank)} className="justify-start" size="sm" variant="ghost"><Archive aria-hidden />Archive</Button> : <Button aria-label={`Reactivate ${bank.name}`} disabled={busy !== null} onClick={() => { void changeStatus(bank, "reactivate"); }} className="justify-start" size="sm" variant="ghost"><RotateCcw aria-hidden />Reactivate</Button>}</div></TableCell>
                      </TableRow>
                    ))}</TableBody>
                  </Table>
                </div>
